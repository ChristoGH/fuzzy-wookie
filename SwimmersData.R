initial.dir<-getwd()
setwd("C:/Users/Admin/Creative Cloud Files/Swimming/SA Short Course Champs Aug PMBG 2014/SwimmersData")
...
ch <- odbcConnect("Swimming")
        eventFrame <- sqlFetch(ch, "eventFrame")
odbcClose(ch) 

uniqueSwimmerList <- unique(eventFrame$Swimmer)



SwimmersDetail <- lapply(X = uniqueSwimmerList, FUN = getSwimmersDetailX, eF=eventFrame)
...
# Work on 8Jan2015:
        pdf(file="Entry number by Event by AgeGroup - Men.pdf")
                ggplot(eventFrame[(!eventFrame$MultiClass)&(!eventFrame$Finals)&(eventFrame$Gender=="M"),], aes(x=reorder(factor(EventString),table(factor(EventString))[factor(EventString)]), fill = factor(AgeGroup))) +
        ...
        pdf(file="Entry number by Event by Province - Men.pdf")
                ggplot(eventFrame[(!eventFrame$MultiClass)&(!eventFrame$Finals)&(eventFrame$Gender=="M"),], aes(x=reorder(factor(EventString),table(factor(EventString))[factor(EventString)]), fill = factor(Province))) +
...
radius <- sqrt( eventFrame[(!eventFrame$MultiClass)&(eventFrame$Finals)&(eventFrame$Gender=="F"),]$points/ pi )

        pdf(file="Entry number by Event by AgeGroup - Women.pdf")
                ggplot(eventFrame[(!eventFrame$MultiClass)&(!eventFrame$Finals)&(eventFrame$Gender=="F"),], aes(x=reorder(factor(EventString),table(factor(EventString))[factor(EventString)]), fill = factor(AgeGroup))) +
        ...
        pdf(file="Entry number by Event by Province - Women.pdf")
                ggplot(eventFrame[(!eventFrame$MultiClass)&(!eventFrame$Finals)&(eventFrame$Gender=="F"),], aes(x=reorder(factor(EventString),table(factor(EventString))[factor(EventString)]), fill = factor(Province))) +
                geom_bar(binwidth=1)+ coord_flip() + labs(title = "Entries by Event - Women", x="Event", y="Entries") +
                scale_fill_discrete(name="Province")+
                theme(
                        axis.title.y = element_text(size = rel(1), colour = "white", face ='bold'),
...
ggplot(eventFrame[(!eventFrame$MultiClass)&(!eventFrame$Finals),], aes(x=reorder(factor(DistanceString),table(factor(DistanceString))[factor(DistanceString)]), fill = factor(Gender))) +
...
bubbleData<-aggregate(points~Province+Distance+Stroke+Gender, data=eventFrame[(as.logical(eventFrame$MultiClass))&(as.logical(eventFrame$Finals))&(eventFrame$Gender=="M"),],sum)
...
bubbleData<-aggregate(points~AgeGroup+Distance+Stroke+Gender+Province, data=eventFrame[(!eventFrame$MultiClass)&(!eventFrame$Finals)&(eventFrame$Gender=="M"),],sum)
...
ggplot(eventFrame[(!eventFrame$MultiClass)&(eventFrame$Finals)&(eventFrame$Gender=="F"),], aes(x=reorder(factor(EventString),table(factor(EventString))[factor(EventString)]), y=points, fill = factor(Province))) +
...
ggplot(eventFrame, aes(x=factor(Distance), fill = factor(AgeGroup))) +
        geom_bar(binwidth=1)
